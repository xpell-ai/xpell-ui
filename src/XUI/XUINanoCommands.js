/**
 * XUI Nano Commands
 *
 * Defines the built-in nano commands available to XUI objects.
 *
 * Nano commands are lightweight, string-addressable commands
 * that can be invoked from UI definitions, attributes, events,
 * or dynamic bindings to trigger runtime behavior.
 *
 * They provide a minimal command layer between declarative UI
 * schemas and imperative runtime logic.
 *
 * ---
 *
 * ## Purpose
 *
 * - Enable declarative event handling in XUI
 * - Allow UI schemas to trigger logic without embedding code
 * - Provide a safe, controlled command surface for runtime execution
 *
 * ---
 *
 * ## Scope
 *
 * - Commands defined here are **UI-scoped**
 * - Navigation-related commands are delegated to **XVM**
 * - Data and entity commands are delegated to **XDB / XObject**
 *
 * ---
 *
 * ## Architectural Rule
 *
 * Nano commands MUST remain:
 * - Small and composable
 * - Side-effect aware
 * - Deterministic in execution
 *
 * @packageDocumentation
 * @since 2022-07-22
 * @copyright
 * © 2022–present Aime Technologies. All rights reserved.
 */
import { XCommand, _xlog, XObject, _xd, XD_FRAME_NUMBER } from "@xpell/core";
import { _xem } from "../XEM/XEventManager";
// import { _xem } from "../XEM/XEventManager"
import XUIObject from "./XUIObject";
/**
 * XUINanoCommand Pack
 */
export const _xuiobject_basic_nano_commands = {
    hide: (_cmd, obj) => obj?.hide?.(),
    show: (_cmd, obj) => obj?.show?.(),
    toggle: (_cmd, obj) => obj?.toggle?.(),
    // canonical text setter
    "set-text": (cmd, obj) => {
        if (!obj)
            return;
        obj._text = String(cmd._params?.text ?? "");
    },
    // keep old alias
    set: (cmd, obj) => {
        if (!cmd._params)
            return;
        if (cmd._params?.text !== undefined) {
            obj._text = String(cmd._params.text ?? "");
        }
    },
    "set-text-from-frame": (cmd, obj) => {
        if (!obj)
            return;
        // Prefer canonical key
        let v = _xd.get(XD_FRAME_NUMBER);
        // Fallback to legacy key if compat legacy keys are enabled
        if (v === undefined && _xd._compat_legacy_keys) {
            v = _xd.get("frame-number");
        }
        let text = v;
        const pattern = cmd._params?.pattern;
        if (pattern)
            text = String(pattern).replace("$data", String(text ?? ""));
        obj._text = String(text ?? "");
    },
    "set-text-from-data": (cmd, obj) => {
        if (!obj)
            return;
        // ✅ data is passed from onData(...) through checkAndRunInternalFunction
        const data = cmd._params?.data;
        if (data === undefined)
            return;
        let text = data;
        const pattern = cmd._params?.pattern;
        if (pattern)
            text = String(pattern).replace("$data", String(data ?? ""));
        const empty = cmd._params?.empty;
        if (empty === true || empty === "true")
            obj.emptyDataSource();
        obj._text = String(text ?? "");
    },
    // ---- New universal UI commands ----
    "add-class": (cmd, obj) => {
        const cls = cmd._params?.class;
        if (obj && cls)
            obj.addClass(String(cls));
    },
    "remove-class": (cmd, obj) => {
        const cls = cmd._params?.class;
        if (obj && cls)
            obj.removeClass(String(cls));
    },
    "toggle-class": (cmd, obj) => {
        const cls = cmd._params?.class;
        if (obj && cls)
            obj.toggleClass(String(cls));
    },
    "set-style": (cmd, obj) => {
        const name = cmd._params?.name;
        const value = cmd._params?.value;
        if (obj && name != null && value != null) {
            obj.setStyleAttribute(String(name), String(value));
        }
    },
    "set-attr": (cmd, obj) => {
        if (!obj)
            return;
        const name = cmd._params?.name;
        const value = cmd._params?.value;
        if (!name)
            return;
        const el = obj.dom;
        if (el?.setAttribute)
            el.setAttribute(String(name), String(value ?? ""));
        else
            obj[String(name)] = value;
    },
    "remove-attr": (cmd, obj) => {
        if (!obj)
            return;
        const name = cmd._params?.name;
        if (!name)
            return;
        const el = obj.dom;
        if (el?.removeAttribute)
            el.removeAttribute(String(name));
        else
            delete obj[String(name)];
    },
    "focus": (_cmd, obj) => {
        const el = obj?.dom;
        if (el?.focus)
            el.focus();
    },
    "scroll-into-view": (cmd, obj) => {
        const el = obj?.dom;
        if (!el?.scrollIntoView)
            return;
        const behavior = cmd._params?.behavior;
        const block = cmd._params?.block;
        el.scrollIntoView({
            behavior: behavior === "smooth" ? "smooth" : "auto",
            block: ["start", "center", "end", "nearest"].includes(block) ? block : "start",
        });
    },
};
export default (_xuiobject_basic_nano_commands);
