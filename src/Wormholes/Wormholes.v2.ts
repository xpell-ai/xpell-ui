...
type Waiter = { _resolve: (v: any) => void; _reject: (e: any) => void; _ts: number };
type WaitersPack = Record<string, Waiter>;

function toStringData(evtData: any) {
  try {
...
export class WormholesV2 implements WormholesClientAPI {
  private _ws: WebSocket | null = null;
  private __ready = false;

  private __wid?: string;
  private __sid?: string;

  private _opts?: WormholesOpenOptions;
  private _waiters: WaitersPack = {};
  private _data_queue: Record<string, any[]> = {};

  private _on_open?: () => void;

  private _log = { _connect: false, _disconnect: false, _send: false, _receive: false, _evt: false };
  ...
  open(opts: WormholesOpenOptions) {
    // helper must be defined before any use
    const _briefFireError = (e: any) => {
      _xlog.error(e);
    ...
    if (this._log._connect) _xlog.log("[WHv2] connecting", url);
    ...
    try {
      ...
      const helloPayload: WHHelloPayload = {
        _protocol: "wormholes",
        _v: 2,
    ...
    this._ws.onmessage = (evt) => {
      try {
        ...
        const env = parseEnvelope(raw) as WHEnvelope<any>;

        if (this._log._receive) _xlog.log("[WHv2] recv", env);

        if (env._wid) this.__wid = env._wid;
        if (env._sid) this.__sid = env._sid;

        // tolerate early-alpha sid inside payload
        const p: any = env._payload ?? {};
        if (p?._sid && !this.__sid) this.__sid = p._sid;
      ...
      if (this._opts?._auto_reconnect) {
        ...
        setTimeout(() => {
          if (!this.__ready && this._opts) this.open(this._opts);
        }, ms);
      }
    };

    this._ws.onerror = (err) => {
      if (this._log._disconnect) _xlog.log("[WHv2] error", err as any);
      _xem.fire(WormholeEvents.WormholeError, { _error: err });
    };
  }

  close() {
    this._opts = { ...(this._opts ?? {}), _auto_reconnect: false } as any;
    try {
      this._ws?.close();
    } catch { }
    this._ws = null;
  ...
  async sendXcmd(xcmd: XCmd, timeoutMs = 20000) {
  ...
  sendSync(payload: any, timeoutMs = 20000): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const env = makeReq(payload as XCmd, this.__wid, this.__sid);

      this._waiters[env._id] = { _resolve: resolve, _reject: reject, _ts: Date.now() };

      try {
        if (this._log._send) _xlog.log("[WHv2] send", env);
  ...
  private _sendEnvelope(env: WHEnvelope) {
    if (!this._ws || !this.__ready) return;
    try {
      if (this._log._send) _xlog.log("[WHv2] send", env);
      this._ws.send(stringifyEnvelope(env));
    } catch (e) {
  ...
  private _handleEnvelope(env: WHEnvelope<any>) {
    switch (env._kind) {
      case "HELLO":
      ...
      case "EVT":
        this._handleEvt(env._payload as any);
        return;
