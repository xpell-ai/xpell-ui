...
export class WormholesV1 extends XModule {
    ...
    createMessage(msg, type = MessageType.JSON) {
        const oData = this.stringify(msg);
        return {
            id: _xu.guid(),
            type,
            data: oData,
    ...
    open(url) {
        ...
        const sthis = this;
        if (this._wh_log_rules._open)
            _xlog.log("[WHv1] opening...");
        ...
        this._ws.onmessage = async (evt) => {
            try {
                ...
                if (this._wh_log_rules._receive)
                    _xlog.log("[WHv1] recv", ddata);
            }
            catch (e) {
                _xlog.error(e);
            }
     ...
     */
    send(message, cb, type = MessageType.JSON) {
        if (!this._ws)
            return;
        const wormholeMessage = this.createMessage(message, type);
        if (!cb) {
            cb = (data) => _xlog.log("[WHv1 default callback] data ->", data);
        }
        this._data_waiters[wormholeMessage.id] = cb;
        try {
            this._ws.send(JSON.stringify(wormholeMessage));
            if (this._wh_log_rules._send)
                _xlog.log("[WHv1] send", wormholeMessage);
        }
        catch (ex) {
     ...
     */
    sendSync(message, checkXProtocol = true) {
        return new Promise((resolve, reject) => {
            this.send(message, (data) => {
                if (!data)
                    return reject("ERROR: No response from server");
                if (!checkXProtocol)
                    return resolve(data);
                if (data["_ok"])
 ...
 */
export class WormholesV1Adapter {
    constructor() {
        this._v1 = new WormholesV1();
    ...
    async sendSync(payload, timeoutMs) {
        ...
        void timeoutMs;
        if (typeof this._v1.sendSync === "function") {
            return this._v1.sendSync(payload, true);
        }
        throw new Error("WormholesV1Adapter: legacy v1 does not implement sendSync()");
    }
    async sendXcmd(xcmd, timeoutMs) {
    ...
    onOpen(cb) {
        if (typeof this._v1.onOpen === "function") {
            this._v1.onOpen(cb);
            return;
        }
